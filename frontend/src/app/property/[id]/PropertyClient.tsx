    ...
    <div className="min-h-screen bg-white flex flex-col font-sans max-w-md mx-auto relative border-x border-slate-100 overflow-hidden h-screen">
      ...
      <div onScroll={handleScroll} className="flex-1 overflow-y-auto no-scrollbar">
        ...
        <div 
           ...
           className="relative h-[380px] w-full bg-slate-100 overflow-hidden cursor-pointer"
        >
          <div 
          ...
          >
             {imagesList.map((img, idx) => (
                <div key={idx} className="w-full h-full shrink-0 snap-center relative">
                   <Image src={img} alt={`${property.title} - ${idx + 1}`} fill className="object-cover" priority />
                </div>
             ))}
          </div>
      ...
      {/* Booking Bar */}
      <div className="absolute bottom-6 left-0 right-0 z-[1002] px-6 flex justify-center pointer-events-none">
      ...
      {/* Modals Container */}
      <div className="fixed inset-0 pointer-events-none z-[20000]">
          ...
          {isGalleryOpen && (
            ...
            >
               ...
               <div className="relative w-full h-full flex items-center justify-center px-2">
                  <button onClick={() => setCurrentImg((prev) => (prev > 0 ? prev - 1 : imagesList.length - 1))} className="absolute left-4 z-20 h-12 w-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white border border-white/10"><ChevronLeft className="h-6 w-6" /></button>
                  <AnimatePresence mode="wait">
                     <motion.div key={currentImg} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.3 }} className="relative w-full h-[65vh]">
                        <Image src={imagesList[currentImg]} alt="Full Gallery" fill className="object-contain" />
                     </motion.div>
                  </AnimatePresence>
                  <button onClick={() => setCurrentImg((prev) => (prev < imagesList.length - 1 ? prev + 1 : 0))} className="absolute right-4 z-20 h-12 w-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white border border-white/10"><ChevronRight className="h-6 w-6" /></button>
               </div>
            </motion.div>
          )}
        </AnimatePresence>
                ...
                {!isBookingSuccess ? (
                  <>
                    <div className="flex items-center justify-between mb-8">
                      ...
                      <button onClick={() => setIsConfirmOpen(false)} className="h-9 w-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-500"><X className="h-5 w-5" /></button>
                    </div>
                  ...
                  <div className="flex flex-col items-center py-6 text-center">
                    <div className="size-16 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mb-4"><CheckCircle2 className="size-8" /></div>
                    <h3 className="text-[20px] font-black text-slate-900 mb-1">Заявка принята!</h3>
                    <p className="text-[13px] font-medium text-slate-400 mb-8">Наш админ свяжется с вами в течение 5 минут.</p>
                    <button onClick={() => { router.push('/trips'); }} className="w-full h-14 bg-slate-900 text-white rounded-[1.25rem] text-[15px] font-black uppercase tracking-widest active:scale-95 transition-all">Готово</button>
                  </div>
                )}
              </motion.div>
            </>
            ...
            <>
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="absolute bottom-0 left-0 right-0 max-w-md mx-auto bg-white rounded-t-[2.5rem] p-6 pb-10 shadow-2xl pointer-events-auto" >
                ...
                <div className="flex items-center justify-between mb-4 px-2">
                  ...
                  <button onClick={handleNextMonth} className="h-9 w-9 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-100"><ChevronRight className="h-4.5 w-4.5 text-slate-600" /></button>
                </div>
        ...
        <AnimatePresence>
          {isLocalGuestOpen && (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsLocalGuestOpen(false)} className="absolute inset-0 bg-black/40 backdrop-blur-sm pointer-events-auto" />
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="absolute bottom-0 left-0 right-0 max-w-md mx-auto bg-white rounded-t-[2.5rem] p-6 pb-10 shadow-2xl pointer-events-auto" >
                <div className="flex items-center justify-between mb-8"><h3 className="text-[18px] font-black text-slate-900">Количество гостей</h3><button onClick={() => setIsLocalGuestOpen(false)} className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500"><X className="h-4 w-4" /></button></div>
                <div className="space-y-6 mb-10">
                   <div className="flex items-center justify-between"><div><h4 className="text-[15px] font-black text-slate-800">Взрослые</h4><p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">От 13 лет</p></div><div className="flex items-center gap-4"><button onClick={() => setAdults(Math.max(1, adults - 1))} className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600"><Minus className="h-4 w-4" /></button><span className="text-[16px] font-black text-slate-900 w-4 text-center">{adults}</span><button onClick={() => setAdults(adults + 1)} className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600"><Plus className="h-4 w-4" /></button></div></div>
                   <div className="flex items-center justify-between"><div><h4 className="text-[15px] font-black text-slate-800">Дети</h4><p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">До 12 лет</p></div><div className="flex items-center gap-4"><button onClick={() => setKids(Math.max(0, kids - 1))} className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600"><Minus className="h-4 w-4" /></button><span className="text-[16px] font-black text-slate-900 w-4 text-center">{kids}</span><button onClick={() => setKids(kids + 1)} className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600"><Plus className="h-4 w-4" /></button></div></div>
                </div>
