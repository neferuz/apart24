    ...
    <div className="fixed inset-0 bg-[#F5F5F7] overflow-hidden flex flex-col justify-end">
      ...
      >
        <div className={`absolute top-0 left-0 right-0 z-[10001] px-5 flex items-center justify-between transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md border-b border-slate-100 shadow-sm pt-4 pb-4' : 'bg-transparent pt-10 pb-4'}`}>
          ...
          <button onClick={() => router.push('/')} className={`h-[40px] w-[40px] rounded-full flex items-center justify-center active:scale-90 transition-all border ${isScrolled ? 'bg-slate-50 border-slate-100 text-slate-600' : 'bg-white/20 backdrop-blur-md border-white/30 text-white'}`}>
